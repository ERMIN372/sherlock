...
export async function POST(req: Request) {
  ...
  const walletId = accounting ? getWalletId(req) : null;
  if (accounting && !walletId) {
  ...
  // Демо-фолбэк: начисляем на сервер сразу (если учёт включён).
  if (!provider) {
    ...
    return NextResponse.json({ mode: "demo", success: true, searches: null });
  }
